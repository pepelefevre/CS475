...
int main( int argc, char* argv[] ){
  ...
  if(rank < ((cmdLineArgs.problemSize*cmdLineArgs.problemSize) /
    (cmdLineArgs.tile_len_x*cmdLineArgs.tile_len_y))) {
  // you need:
...
fprintf(stderr, "Rank(%d) TileX(%d) TileY(%d) UBX(%d) UBY(%d)\n", rank, tile_x, tile_y, upperBound_x, upperBound_y);
...
//START NORTH/SOUTH
    if (tile_y == 0) {
      ...
      //Only send/receive bottom row
      MPI_Send(space[read][y_ub-1], tile_len_x+2, MPI_DOUBLE, rank+x_tile_count, 0, MPI_COMM_WORLD);
      MPI_Recv(space[read][y_ub], tile_len_x+2, MPI_DOUBLE, rank+x_tile_count, 0, MPI_COMM_WORLD, &s);

    } else if (tile_y == y_tile_count-1) {
      
      //Only send/recieve top row
      MPI_Recv(space[read][0], tile_len_x+2, MPI_DOUBLE, rank-x_tile_count, 0, MPI_COMM_WORLD, &s);
      MPI_Send(space[read][y_lb], tile_len_x+2, MPI_DOUBLE, rank-x_tile_count, 0, MPI_COMM_WORLD);

    } else {

      //Send/receive both top and bottom
      MPI_Recv(space[read][0], tile_len_x+2, MPI_DOUBLE, rank-x_tile_count, 0, MPI_COMM_WORLD, &s);
      MPI_Send(space[read][y_lb], tile_len_x+2, MPI_DOUBLE, rank-x_tile_count, 0, MPI_COMM_WORLD);
      MPI_Send(space[read][y_ub-1], tile_len_x+2, MPI_DOUBLE, rank+x_tile_count, 0, MPI_COMM_WORLD);
      MPI_Recv(space[read][y_ub], tile_len_x+2, MPI_DOUBLE, rank+x_tile_count, 0, MPI_COMM_WORLD, &s);
...
//START E/W
    if (tile_x == 0) {
      ...
      //Only send/receive right column
      packColToVec(space[read], x_ub-1, buffer, tile_len_y+2);
      MPI_Send(buffer, tile_len_y+2, MPI_DOUBLE, rank+1, 0, MPI_COMM_WORLD);
      MPI_Recv(buffer, tile_len_y+2, MPI_DOUBLE, rank+1, 0, MPI_COMM_WORLD, &s);
      unpackVecToCol(space[read], x_ub, buffer, tile_len_y+2);

    } else if (tile_x == x_tile_count-1) {
      
      //Only send/recieve left column
      MPI_Recv(buffer, tile_len_y+2, MPI_DOUBLE, rank-1, 0, MPI_COMM_WORLD, &s);
      unpackVecToCol(space[read], x_lb-1, buffer, tile_len_y+2);
      packColToVec(space[read], x_lb, buffer, tile_len_y+2);
      MPI_Send(buffer, tile_len_y+2, MPI_DOUBLE, rank-1, 0, MPI_COMM_WORLD);
